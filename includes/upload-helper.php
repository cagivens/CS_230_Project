...
if (isset($_POST['prof-submit'])) {
    ...
    $sql = "UPDATE profile SET picpath=?, bio=? WHERE username=?;";
    $stmt = mysqli_stmt_init($conn);

    if(!mysqli_stmt_prepare($stmt, $sql)) {
        header("Location: ../profil.php?error=SqlInjectionError");
        exit();
    }

    mysqli_stmt_bind_param($stmt, "sss", $destination, $bio, $username);
